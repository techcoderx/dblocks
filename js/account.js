...
export default class extends view {
    ...
    init() {
        axios.get(node_address + '/account/' + this.account).then((acc) => {
            ...
            axios.get(node_address+'/rewards/pending/' + this.account).then((pending) =>
                $('#acc-meta-pending').text(thousandSeperator(Math.floor(pending.data.total) / 100) + ' DTC'))
            .catch(()=>
                $('#acc-meta-pending').text('Error'))
    
            axios.get(node_address + '/rewards/claimed/' + this.account).then((claimed) =>
        ...
        let accountHistoryUrl = node_address + '/history/' + this.account + '/0'
    ...
    updateLeaderStats() {
        ...
        axios.get(node_address + '/leader/' + this.account).then((leader) => {
